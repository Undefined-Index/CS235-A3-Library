...
class Review:
    ...
    def __init__(self, book: Book, review_text: str, rating: int, user: str):
...
class User:
    ...
    def __init__(self, user_name: str, password: str):
        ...
        if password == "" or not isinstance(password, str) or len(password) < 8:
    ...
    @property
    def want_to_read(self) -> str:
        return self.__want_to_read
    ...
    def add_book_to_want_to_read(self, Bid: str):
        if isinstance(Bid, str):
            temp_list = self.__want_to_read.split(",")
            if Bid not in temp_list:
                temp_list.append(Bid)
                self.__want_to_read = ','.join(temp_list)

    def remove_book_to_want_to_read(self, Bid: str):
        if isinstance(Bid, str):
            temp_list = self.__want_to_read.split(",")
            if str(Bid) in temp_list:
                temp_list.remove(str(Bid))
                self.__want_to_read = ','.join(temp_list)
