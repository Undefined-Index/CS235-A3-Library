...
class TestReview:
    ...
    def test_attributes_access(self):
        book = Book(2675376, "Harry Potter")
        review = Review(book, 42, 3,'testacc')
        assert str(review.book) == "<Book Harry Potter, book id = 2675376>"
    ...
    def test_invalid_parameters(self):
        ...
        with pytest.raises(ValueError):
            review = Review(book, review_text, -1,'testacc')

        with pytest.raises(ValueError):
            review = Review(book, review_text, 6,'testacc')

    def test_set_of_reviews(self):
        book1 = Book(2675376, "Harry Potter")
        book2 = Book(874658, "Lord of the Rings")
        review1 = Review(book1, "I liked this book", 4,'testacc')
        review2 = Review(book2, "This book was ok", 3,'testacc')
        review3 = Review(book1, "This book was exceptional", 5,'testacc')
    ...
    def test_wrong_book_object(self):
        ...
        review = Review(publisher, "I liked this book", 4,'testacc')
...
class TestUser:
    ...
    def test_construction(self):
        ...
        user1 = User('Shyamli', 'pw123456')
        user2 = User('Martin', 'pw678901')
        user3 = User('Daniel', 'pw874656')
    ...
    def test_sort_ordering(self):
        user1 = User("Shyamli", "pw123456")
        user2 = User("Martin", "pw678901")
        user3 = User("daniel", "pw123456")
        assert user1 > user2
        assert user1 > user3
        assert user2 > user3

    def test_comparison(self):
        user1 = User("Martin", "pw123456")
        user2 = User("Shyamli", "pw678901")
        user3 = User("martin", "pw456734")
        assert user1 == user3
        assert user1 != user2
        assert user3 != user2
    ...
    def test_passwords(self):
        ...
        assert str(user1) == "<User shyamli>"
        assert str(user1.password) == "pw123456"
        assert str(user2) == "<User martin>"
        assert user2.password is None

    def test_add_want_to_read(self):
        #can add book with valid id and not in want to read list to want to read list
        book_id = '874658'
        user = User("Martin", "pw123456")
        user.add_book_to_want_to_read(book_id)
        want_to_read_list = user.want_to_read.split(",")
        want_to_read_list.pop(0) # remove ''
        assert len(want_to_read_list) == 1
        assert want_to_read_list[0] == book_id

        #cannot add book already in want to read list again to want to read list
        user.add_book_to_want_to_read(book_id)
        want_to_read_list = user.want_to_read.split(",")
        want_to_read_list.pop(0) # remove ''
        assert len(want_to_read_list) == 1
        ...
        #cannot add book with invalid id to want to read list
        book_id2 = 123
        user.add_book_to_want_to_read(book_id2)
        want_to_read_list = user.want_to_read.split(",")
        want_to_read_list.pop(0) # remove ''
        assert len(want_to_read_list) == 1
        assert want_to_read_list[0] == book_id
    ...
    def test_remove_want_to_read(self):
        ...
        book_id2 = '384923'
        book_id3 = '402849'
        books_id = [book_id1, book_id2, book_id3]
        for book_id in books_id:
            user.add_book_to_want_to_read(book_id)

        #can remove book already in want to read list
        user.remove_book_to_want_to_read('874658')
        want_to_read_list = user.want_to_read.split(",")
        want_to_read_list.pop(0) # remove ''
        assert len(want_to_read_list) == 2
        assert want_to_read_list[0] == '384923'
        assert want_to_read_list[1] == '402849'

        #can't remove book if a valid book id is given but that book is not in want to read list
        user.remove_book_to_want_to_read('573820')
        want_to_read_list = user.want_to_read.split(",")
        want_to_read_list.pop(0) # remove ''
        assert len(want_to_read_list) == 2
        assert want_to_read_list[0] == '384923'
        ...
        #can't remove book if an invalid book id is given
        user.remove_book_to_want_to_read(123)
        want_to_read_list = user.want_to_read.split(",")
        want_to_read_list.pop(0) # remove ''
        assert len(want_to_read_list) == 2
        assert want_to_read_list[0] == '384923'
